...
"Two linear transformations with a ReLU activation in between." Expand, rectify, contract: $W_1$ lifts the token's vector from the base model's $d_\text{model} = 512$ to $d_\text{ff} = 2{,}048$, the ReLU (rectified linear unit) zeroes the negatives, $W_2$ brings it back down. The 4× expansion comes with no derivation; GPT-3 canonized it anyway. A habit, not a theorem.
...
- **GELU** (Gaussian error linear unit; [Hendrycks & Gimpel, 2016](https://arxiv.org/abs/1606.08415)) — $x \cdot \Phi(x)$, a smoothed ReLU with gradient below zero. GPT adopted it, then BERT ("We use a gelu activation... following OpenAI GPT"). The same paper introduced **SiLU**, $x \cdot \sigma(x)$, which [Swish](https://arxiv.org/abs/1710.05941) (Ramachandran et al., 2017) rediscovered by automated search.
