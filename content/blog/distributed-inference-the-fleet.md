...
Past a certain load you run out of machine, and a bigger GPU won't save you — the biggest is already in the node. The fix is more of them: a fleet. That is the whole subject of this series — high-volume, distributed LLM inference — and this first post is just the map: the boxes a production fleet is made of, and the two planes they split across. Later posts open each box.

A fleet of GPUs serving one model is a distributed system like any other — just with unusually expensive, unusually stateful workers. These are my notes on its shape.
...
The way past a full machine is more replicas behind a load balancer, the way you'd scale a stateless web service. Except inference replicas aren't stateless. A replica that just processed a long prompt is holding that prompt's KV cache; the next turn of the same conversation wants that cache, and only that replica has it. Round-robin throws it away.
