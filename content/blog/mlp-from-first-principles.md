...
The original feed-forward network is one line — Section 3.3 of [*Attention Is All You Need*](https://arxiv.org/abs/1706.03762) (Vaswani et al., 2017):

$$
\mathrm{FFN}(x) = \max(0,\; xW_1 + b_1)\,W_2 + b_2
$$

"Two linear transformations with a ReLU activation in between." Expand, kink, contract: $W_1$ lifts the token's vector from the base model's $d_\text{model} = 512$ to $d_\text{ff} = 2{,}048$, the ReLU zeroes the negatives, $W_2$ brings it back down. The 4× expansion comes with no derivation; GPT-3 canonized it anyway. A habit, not a theorem.

The property that matters most: the FFN is applied "to each position separately and identically" — it is **position-wise**. Every token goes through alone, same weights, and no token sees any other; attention is the transformer's *only* cross-token operation. Two inference consequences: no KV cache — nothing about other tokens to remember — and constant cost per token. Just the weights, read again.
...
The intuition is cheaper than the theorem. A softmax row is non-negative and sums to one, so every attention output is a **convex combination** of value vectors — a weighted average, and averages of averages blur. Attention *moves* information; it doesn't transform it. Honesty note: the paper credits skip connections as the main antidote; MLPs only *slow* the collapse. The division of labor stands: attention mixes, only the MLP nonlinearly rebuilds a token's vector.

![Figure 2 — Mixing versus transforming, in three steps. One attention step outputs a softmax-weighted average of value vectors — weights non-negative, summing to 1, so the output lands inside the hull of its inputs. Stack attention alone and the hulls shrink layer by layer: Dong et al. prove collapse to rank-1 "token uniformity" at a doubly exponential 3^L rate. The MLP is the escape — per-token and nonlinear, it moves a vector outside the hull, manufacturing features instead of averaging them.](/assets/blog/mlp-from-first-principles/mixing-vs-transforming.svg)

The MLP's parameters also appear to be where the model keeps what it knows. [Geva et al. (EMNLP 2021)](https://arxiv.org/abs/2012.14913) read the FFN as a **key–value memory**: the first matrix's directions act as keys matching input patterns (shallow in lower layers, semantic higher up), the second's as values pushing distributions over the output vocabulary. [ROME](https://arxiv.org/abs/2202.05262) (Meng et al., NeurIPS 2022) makes it causal: a rank-one edit to one *mid-layer* MLP matrix rewrites a specific fact in GPT-2 XL — edit one matrix, change one thing the model believes. (A caution against reading too literally: Anthropic's superposition work ([Elhage et al., 2022](https://arxiv.org/abs/2209.10652)) shows MLPs pack more features than they have neurons, so a single neuron rarely *means* one thing.) That's the machinery worth opening up — matrix by matrix.

## The math: match, gate, write back
...
**Match.** $xW_\text{gate}$ scores the token against $28{,}672$ learned patterns — each column of $W_\text{gate}$ is a direction in the 8,192-dimensional residual space, and the dot product measures how strongly this token exhibits it. This is Geva's key lookup, one score per hidden neuron.
...
**Write back.** $W_\text{down}$ has $28{,}672$ rows, each a direction in residual space — Geva's values. The output is the activation-weighted sum of those rows, added back into the residual stream. The token leaves the block moved by whichever stored directions its patterns switched on.

![Figure 3 — SwiGLU dataflow for one Llama 2 70B token: match, gate, write back. The 8,192-wide token vector is projected by W_gate and W_up into two 28,672-wide vectors; SiLU turns the gate path into per-neuron open/closed scores that multiply the up path elementwise; W_down writes the surviving activations back as a weighted sum of its 28,672 output directions, added to the residual stream. Three 8,192 × 28,672 matrices — 704.6M parameters per layer — and no KV cache anywhere.](/assets/blog/mlp-from-first-principles/swiglu-dataflow.svg)

> [!NOTE]
> Why is Llama's $d_\text{ff}$ 28,672 and not $4 \times 8{,}192 = 32{,}768$? Parameter matching. The vanilla FFN has two matrices, $2 \times d \times 4d = 8d^2$ parameters. A gated FFN has three, so Shazeer shrank the hidden width "by a factor of 2/3": at $d_\text{ff} = \tfrac{2}{3} \cdot 4d = \tfrac{8}{3}d$ you get $3 \times d \times \tfrac{8}{3}d = 8d^2$ — identical parameters and FLOPs (his runs: 3,072 → 2,048). LLaMA adopted the rule verbatim ("We use a dimension of 2/3 4d instead of 4d as in PaLM"), and later models drifted wide of it: Llama 2/3 70B at 3.5×, Llama 3 405B at 3.25×, Qwen2.5-72B at 3.61×. The 2/3 was never about quality — an accounting convention that became architecture.

Now the bill. Per layer, $3 \times 8{,}192 \times 28{,}672 = 704{,}643{,}072$ parameters; across 80 layers, **56.37B** — 81.7% of the model. A forward pass costs about 2 FLOPs per parameter per token ([Kaplan et al., 2020](https://arxiv.org/abs/2001.08361)): ~1.41 GFLOPs per layer, ~112.7 GFLOPs per token in FFNs alone. At batch-1 decode each matmul is a **GEMV** — every weight read once and used once, about 1 FLOP per byte at fp16, against an H100 ridge near 295. Deeply memory-bound: streaming the 112.7 GB of fp16 FFN weights at 3.35 TB/s costs ~33.6 ms per token before any math. Batch requests or run prefill and the same read serves $S$ tokens — the GEMV becomes a **GEMM**, intensity scales by $S$, and the op flips compute-bound. The first post's asymmetry, pinned to the exact matrices that cause it.

## The activation: why a kink is load-bearing

Delete the activation and depth evaporates: $(xW_1)W_2 = x(W_1 W_2)$. Two linear maps compose into one, eighty layers of them compose into one, and the network collapses to a single matrix. That one line is the whole argument for nonlinearity — almost any kink restores expressiveness (one hidden layer is universal *iff* the activation is not a polynomial; Leshno et al., 1993). Which kink matters in practice, and the lineage is short:
...
Meta's [Memory Layers at Scale](https://arxiv.org/abs/2412.09764) takes Geva literally and *replaces* some layers' FFN with a trainable key–value lookup — 128B memory parameters, author-reported factual-QA gains over compute-matched dense baselines. [KAN](https://arxiv.org/abs/2404.19756) rebuilds the neuron itself around learnable spline activations; validated on small scientific tasks, used by no frontier LLM.

| Direction | What it changes | Representative work | Honest status |
| --- | --- | --- | --- |
| Replicate it (MoE) | many FFNs, few active per token | Mixtral, DeepSeek-V3, Llama 4, Qwen3 | shipped across the frontier |
| Skip the dead neurons | read only active weights | Deja Vu, PowerInfer, TurboSparse | research + local inference; wants ReLU back |
| Change the kink | ReLU² for sparsity and speed | Primer, Nemotron-H | shipped (NVIDIA); minority choice |
| Replace it with lookup | FFN → trainable memory | Memory Layers at Scale | research, author-reported |
